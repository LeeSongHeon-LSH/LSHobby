...
- **Gemini API = 결제수단 미연결 무료 등급**(§16.13): 과금이 구조적으로 불가능한 상태를 유지한다. 2026-09-09에 OpenAI·Anthropic 유료 키로 백필을 옮기는 안을 검토했으나 **보류** — 붙일 자리가 백필 배치뿐이고 총 호출이 100회 남짓이라 무료 한도로 며칠 더 도는 쪽을 택했다. 재론 조건: 백필이 한 달 넘게 안 끝나거나 새 언어가 추가될 때
- **DB는 원격 Supabase 유지 — 로컬 DB 전환 보류** (2026-09-09 검토): "궤적이 쌓이면 무료 한도를 넘을 것"이라는 우려로 로컬 DB + GitHub 백업 + 미니PC 이전 안을 검토했으나 **보류**. 실측이 전제를 뒤집는다 — 2026-09-09 기준 pg_dump 76KB · 17표 약 1,500행 · 복습 로그 221행/20일. 복습 1회 ≈ 100바이트라 하루 500회여도 연 20MB 미만, 500MB 한도는 이 쓰기 속도로는 사실상 무한이다. 무료 플랜의 실제 위험은 용량이 아니라 7일 무활동 일시정지인데 §16.14 타이머가 매일 DB를 건드려 걸리지 않는다. 구조상 전환은 재배포가 아니라 **재작성**이다 — 브라우저→PostgREST 직행 호출 59곳과 RLS 보안 경계를 서버 코드·세션으로 옮겨야 하고, 코드를 안 건드리려면 Supabase 스택 셀프호스트(컨테이너 8개)뿐이다. **DB 파일을 git에 넣는 백업은 채택하지 않는다** — 바이너리라 diff 없이 히스토리만 부풀고, 생각 데이터 외부 반출 금지(§16.11)와 충돌한다(private 리포도 외부다). 지금 구조가 이미 "원본 Supabase + 사본 집 PC(주 1회 복원 리허설, §16.14)" 두 벌이며, 미니PC 이전은 앱 배포 + 타이머 설치로 끝나고 데이터 이사가 없다. 재론 조건: pg_dump가 100MB를 넘을 때, Supabase가 무료 플랜 조건을 바꿀 때, 인터넷 없이 집 안에서만 써야 할 때. 그때의 경로는 락인 회피 원칙대로 `pg_dump` 복원 → 셀프호스트 Supabase 또는 서버 코드 전환

**Google AI Pro 학생 플랜 만료 대비 (2027-09 무렵, 12개월 무료 → 유료 자동 전환)**

이 플랜은 **이 프로젝트의 어떤 코드·키와도 연결돼 있지 않다.** Google 문서 기준으로 AI Pro/Plus 혜택은 AI Studio **웹 화면 안에서만** 적용되고, API 키를 통한 Gemini API 사용은 별도로 관리된다 — 즉 `GEMINI_API_KEY`의 무료 등급 한도(실측 ~20회/일)는 플랜이 끝나도 그대로다. 만료 시 할 일은 코드가 아니라 **계정 쪽**이다.

| 시점 | 할 일 | 안 하면 |
|---|---|---|
| 갱신일 **전** (정확한 날짜는 Google One 구독 페이지) | 구독 **해지** — 가입한 경로(Google Play 또는 Google One 웹)의 구독 관리에서 취소. 해지해도 남은 무료 기간은 유지된다 | 월 요금(AI Pro $19.99 / AI Plus $4.99 상당)이 자동 청구 — "과금 불가" 원칙 위반 |
| 갱신일 전 | Drive 용량이 5TB(Pro)/200GB(Plus) → 15GB로 내려가므로 Drive에 뭘 두고 있다면 정리. **이 프로젝트의 백업은 Drive를 쓰지 않는다**(`~/.lshobby/backups`, §16.14) | 용량 초과 시 Gmail·Drive 쓰기 정지 |
| 만료 **후** | `backfill-*.mjs`를 평소처럼 돌려 429 양상이 바뀌지 않았는지 한 번 확인 | — (무관하다는 문서 근거는 위) |

기록 이유: 1년 뒤에 "학생 플랜이 끝나면 Gemini 키가 죽나?"를 다시 조사하지 않기 위해서다. 답은 "아니다, 해지만 하면 된다".
...
| 쓰기 순서 | 본문 블록 교체 → **그 뒤에** `sync_hash` (2026-09-11, #94) | hash를 먼저 찍으면 본문 교체가 중간에 끊겼을 때 빈 페이지에 hash만 남아 이후 실행이 영원히 건너뛴다 — 감상·회독 기록의 유일한 오프사이트 사본이 사라지는 경로였다. 뒤에 찍으면 끊긴 회차는 다음 실행이 다시 채운다. `children`은 한 요청에 100개까지라 100개씩 나눠 append |
