...
flowchart LR
    L[로그인] --> H[홈 허브]
    H --> LA["언어 세션<br/>탭: 홈·학습·단어장·통계·추가"]
    H --> LI["책 세션<br/>탭: 홈·서재·인용구"]
    H --> KN["CS 세션<br/>탭: 홈·개념"]
    LA -- 시작하기 --> QZ[퀴즈]
    LI --> BD[책 상세]
    KN --> CD[개념 상세]
    CD --> CE[개념 편집]
...
- 세션 간 직접 이동 없음 — 항상 홈 경유 (§11.1). 상세·편집은 탭바 위 푸시 화면
- 홈 타임라인에서 원본이 삭제된 항목은 이동 불가(비활성, §9.3)
...
flowchart TD
    ...
    D --> E[책 상세로 이동]
    E --> F["인용구·노트·reflection<br/>자유 순서, 전부 선택"]
...
```mermaid
flowchart TD
    A["엔티티 삭제<br/>(book / es_word / concept)"] --> B["DB: 진짜 FK 자식은<br/>on delete cascade<br/>(reading·quote·review_log·<br/>sentences·concept_link …)"]
