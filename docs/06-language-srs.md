...
- **출제 순서** (`session.practiceOrder`, 2026-08-31 개편): ① due 지난 복습 → ② 신규 → ③ 아직 due 아닌 것.
  각 구간은 정답률 오름차순 → 오래 안 본 순 → 랜덤. 신규는 집계가 없어 0%가 되므로 별도 판정이 필요 없다.
  복습은 유한하고 신규는 무한하므로 복습을 앞에 둔다 — 반대로 두면 신규가 매 세션 앞을 채워 복습이 굶는다.
