  ...
  if (missing)
    return (
      <main className="p-4 text-center">
        <p className="mt-16 text-neutral-500">책을 찾을 수 없습니다</p>
    ...
    <main className="space-y-6 p-4">
      <header>
        <div className="flex items-start justify-between">
          <Link href="/library" className="py-1 pr-3 text-neutral-500">←</Link>
          <button onClick={openMetaEdit} aria-label="수정" className="py-1 pl-3 text-neutral-500">✎</button>
        </div>
      ...
      <section>
        <h2 className="mb-2 text-sm font-medium text-neutral-500">회독</h2>
        <ul className="rounded-xl border border-neutral-200 bg-white px-4 py-2 text-sm">
          {readings.map((r, i) => (
            <li key={r.id} className="flex justify-between py-1.5">
              <span>{readings.length - i}회독 · {fmtMonth(r.finished_on)}</span>
      ...
      <section>
        <div className="mb-2 flex items-center justify-between">
          <h2 className="text-sm font-medium text-neutral-500">노트</h2>
          <button
            onClick={() => {
        ...
        {editingNote ? (
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <button onClick={() => setEditingNote(false)} className="rounded-lg border border-neutral-300 px-4 py-2 text-sm">취소</button>
              <button onClick={submitNote} disabled={busy} className="flex-1 rounded-lg bg-neutral-900 py-2 text-sm font-medium text-white disabled:opacity-40">저장</button>
            </div>
          </div>
        ) : book.note ? (
          ...
          <p className="text-sm text-neutral-400">아직 노트가 없습니다</p>
        )}
      </section>

      <section>
        <div className="mb-2 flex items-center justify-between">
          <h2 className="text-sm font-medium text-neutral-500">인용구 ({quotes.length})</h2>
          <button onClick={() => setAddingQuote(true)} className="text-sm text-neutral-400">＋</button>
        </div>
        {addingQuote && (
          <div className="mb-3 space-y-2 rounded-xl border border-neutral-200 bg-white p-4">
            <textarea
            ...
            <div className="flex gap-2">
              <button onClick={() => setAddingQuote(false)} className="rounded-lg border border-neutral-300 px-4 py-2 text-sm">취소</button>
              <button onClick={submitQuote} disabled={busy || !quoteForm.content.trim()} className="flex-1 rounded-lg bg-neutral-900 py-2 text-sm font-medium text-white disabled:opacity-40">추가</button>
            </div>
          </div>
        )}
        <ul className="space-y-2">
          {quotes.map((q) => (
            <li key={q.id} className="rounded-xl border border-neutral-200 bg-white p-4 text-sm">
              <p className="leading-relaxed">“{q.content}”{q.page ? <span className="ml-1 text-xs text-neutral-400">p.{q.page}</span> : null}</p>
              {q.comment && <p className="mt-1 text-xs text-neutral-500">└ {q.comment}</p>}
            </li>
          ))}
        </ul>
      </section>
      ...
      {editingMeta && (
        <div className="fixed inset-0 z-10 flex items-end bg-black/30" onClick={() => setEditingMeta(false)}>
          <div className="mx-auto w-full max-w-md space-y-2.5 rounded-t-2xl bg-white p-5 pb-8" onClick={(e) => e.stopPropagation()}>
            {(
              [
                ["title", "제목"],
                ["author", "저자"],
            ...
            <div className="flex gap-2 pt-1">
              <button onClick={remove} disabled={busy} className="rounded-lg border border-red-300 px-4 py-2.5 text-sm text-red-600 disabled:opacity-50">삭제</button>
              <button
                onClick={saveMeta}
